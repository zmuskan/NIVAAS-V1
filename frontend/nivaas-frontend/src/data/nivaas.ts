...
export function rankLocalities(answers: Answers): Match[] {
    const budgetValueMap: Record<BudgetKey, number> = {
        under15: 15000,
        "15to25": 25000,
        "25to40": 40000,
        "40to60": 60000,
    ...
    const ceiling = answers.budget ? budgetValueMap[answers.budget] : 80000;
    const anchor = anchorFor(answers.workArea);
    const remote = isRemote(answers.workArea);
    const workNear = anchor?.near ?? [];

    const scored = localities.map((l) => {
        const reasons: Reason[] = [];
        let score = 0;

        const entry = Number(l.rentRange.replace(/[^\d–-]/g, "").split(/[–-]/)[0] ?? 0);
        if (entry && entry <= ceiling) {
            score += 30;
            reasons.push(
                l.avgRent <= ceiling
                    ? "Sits comfortably inside what you'd like to spend"
                    : "Homes here start below your number",
            );
        } else if (l.avgRent <= ceiling * 1.25) {
            score += 12;
            reasons.push("A slight stretch, but close to your number");
        }

        const nearIndex = workNear.indexOf(l.id);
        if (remote) {
            score += 6;
            if (answers.workArea) reasons.push("Works when your day doesn't start with a commute");
        } else if (nearIndex === 0) {
            score += 26;
            reasons.push(`Close to ${answers.workArea?.trim()}, where your day starts`);
        } else if (nearIndex > 0) {
            score += 16;
            reasons.push(`An easy run to ${answers.workArea?.trim()}`);
        }

        const overlap = answers.priorities.filter((p) => l.traits.includes(p));
        score += overlap.length * 9;
        for (const p of overlap) reasons.push(priorityReason[p]);

        if (answers.lifestyle && l.lifestyleFit.includes(answers.lifestyle)) {
            score += 14;
            reasons.push(lifestyleReason[answers.lifestyle]);
        }

        return { locality: l, reasons: Array.from(new Set(reasons)).slice(0, 3), score };
    });

    return scored
        .sort((a, b) => b.score - a.score)
        .slice(0, 3)
        .map(({ locality, reasons }) => ({ locality, reasons }));
}

const priorityReason: Record<PriorityKey, string> = {
    affordable: "Fits your preference for rent that stays light",
    quiet: "Fits your preference for quieter streets",
    choices: "Plenty to look at, so you're not settling",
    spacious: "Homes here are usually bigger for the money",
    furnished: "You can move in without buying furniture",
    active: "There's life on the street in the evenings",
};

const lifestyleReason: Record<LifestyleKey, string> = {
    student: "Suits students — sharing and short leases are normal here",
    professional: "Works well for someone working long weekdays",
    couple: "A comfortable first neighbourhood for two",
    family: "Set up for families more than for nightlife",
};
