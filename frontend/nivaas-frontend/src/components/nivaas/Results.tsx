...
import { inr, matchLabels, type Answers, type Locality, type Match } from "@/data/nivaas";
            ...
            <div className="relative mx-auto max-w-6xl">
                ...
                >
                    <p className="track-wide text-[0.55rem] text-accent">
                        {answers.name ? `${answers.name}, here's your Bangalore` : "Here's your Bangalore"}
                    </p>
                    <h2 className="mt-6 text-4xl leading-[1.05] text-foreground sm:text-6xl">
                        Three neighbourhoods where
                        <span className="text-dusk-gradient"> your home already exists</span>
                    </h2>
                    <p className="mt-6 text-sm font-light text-muted-foreground sm:text-base">
                        Ranked on what you told us — your budget, your{" "}
                        {answers.workArea ?? "work area"} run, your priorities and who's moving in.
                        Every number below describes real rental inventory, nothing else.
                    </p>
                </motion.div>
                                ...
                                <div className="p-7 sm:p-9 md:col-span-3">
                                    <div className="flex flex-wrap items-baseline justify-between gap-3">
                                        <div>
                                            ...
                                            <p className="mt-1.5 text-xs text-accent">{m.locality.district}</p>
                                        </div>
                                    ...
                                    <div className="mt-7 grid gap-5 sm:grid-cols-3">
                                        <Fact label="Homes listed" value={m.locality.listings.toLocaleString("en-IN")} />
                                        <Fact label="Availability" value={m.locality.availability} />
                                        <Fact label="Rent range" value={m.locality.rentRange} />
                                    </div>

                                    <div className="mt-7">
                                        <p className="track-wide text-[0.5rem] text-accent">Why this fits you</p>
                                    ...
                                    <div className="mt-8 rounded-2xl bg-black/60 backdrop-blur-md p-5">
                                        <p className="track-wide text-[0.5rem] text-muted-foreground">
                                            Imagine living here
                                        </p>
                                        <p className="mt-3 text-base font-light leading-relaxed text-foreground">
                                            {m.locality.imagine.join(" ")}
                                        </p>
                                    </div>

                                    <button
                                        onClick={() => onSelect(m.locality)}
...
function Fact({ label, value }: { label: string; value: string }) {
    return (
        <div>
            <p className="track-wide text-[0.5rem] text-muted-foreground">{label}</p>
